...
const DonationSectionSkeleton = () => (
  <div className="h-60 bg-gray-200 rounded-xl animate-pulse my-8 sm:my-12 md:my-16" />
);

// Memoized components
const VideoCard = memo(
  ({ content, onClick }: { content: TeachingType; onClick: () => void }) => {
...
const VideoModal = memo(
  ({ videoId, onClose }: { videoId: string | null; onClose: () => void }) => {
    const { colorScheme } = useTheme();
...
const MinistryHeader = memo(({ colorScheme }: { colorScheme: any }) => (
  <header className="mb-8 sm:mb-12 md:mb-16 text-center">
    ...
    >
      <SemiBoldText
        ...
        useThemeColor={false}
      >
        Minister Claudy's passion for sharing the Gospel radiates through both
        her writing and speaking. As a gospel artist, devoted teacher, and lover
        of God, she has also shared her inspiring presence on national
        television.
      </SemiBoldText>
    </motion.div>
    ...
    <div className="max-w-4xl mx-auto">
      <div className="flex items-start">
        ...
        >
          ...
          sword. Through these teachings, may your faith be strengthened and
          your spirit renewed.
        </LightText>
      </div>
      <SemiBoldText
        ...
        >
          {visibleItems.map(content => (
            <VideoCard
              key={content.id}
              content={content}
              onClick={() => onVideoSelect(content.youtubeId)}
            />
          ))}
        </motion.div>
  ...
  >
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
      ...
      >
        ...
        >
          Subscribe to receive updates on new teachings, podcasts, and ministry
          events
        </RegularText>
        <NewsletterForm />
      </motion.div>
...
export const MinistryData = () => {
  ...
  const totalSlides = useMemo(
    () => Math.ceil(teachingsData.length / itemsPerPage),
    [itemsPerPage]
  );
  ...
  return (
    <div style={{ backgroundColor: colorScheme.background }}>
      ...
      <VideoModal videoId={selectedVideo} onClose={handleVideoClose} />
      ...
      <article className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 sm:py-12 md:py-16">
        <MinistryHeader colorScheme={colorScheme} />
...
export default memo(MinistryData);
